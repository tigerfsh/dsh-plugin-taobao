...
async function ensureLoginPage(cdp) {
  ...
  if (!/login\.taobao\.com|havanaone/.test(url)) {
...
// minutes, so a static screenshot would be stale by the time it is scanned.
...
<p class="small">本页每 3 秒检查一次，二维码失效会自动换成最新的；登录成功后这里会变成 ✅。</p>
</div><script>
let rev = null
async function tick(){
  let s
